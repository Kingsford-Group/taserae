...
def augment_row(row, feature, sim_dict):
    ...
    if isinstance(original_val, str) and ("," in original_val):
        ...
        # logic from ishikawa/similarity.ipynb
        for idx, item in enumerate(items):
            if item in sim_dict and len(sim_dict[item]) > 0:
                new_row = row.copy()
                replacement = sim_dict[item][0] # Uses the first similar item? Notebook checked: yes, sim_dict[item][0]
...
def main():
    parser = argparse.ArgumentParser(description="Augment data using Ishikawa (Similarity) method")
    ...
    parser.add_argument("--multiplier", type=int, default=3, help="Multiplier for Target_Multiplier strategy")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    
    args = parser.parse_args()
    augment_utils.seed_everything(args.seed)
    
    # Load Data (Strict Split)
    ...
    print(f"Augmentation Targets ({args.strategy}): {sum(targets.values())} samples across {len(targets)} classes.")
    
    # Load Embeddings
    print(f"Loading embeddings from {args.embeddings}...")
    embeddings = np.load(args.embeddings, allow_pickle=True).item()
    
    # Compute Similarity Dicts
    ...
    for label, target in targets.items():
        ...
        while generated_for_label < target:
            # Shuffle group to randomize which rows we pick if we loop multiple times
    ...
    print(f"Total augmented rows generated: {total_generated}")
    augment_utils.save_augmented_data(train_df, augmented_rows, "ishikawa", args.strategy)
