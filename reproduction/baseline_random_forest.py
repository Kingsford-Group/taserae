#!/usr/bin/env python3
"""
Random Forest Baseline for TASER-AE Benchmark

This script trains a Random Forest classifier on the non-augmented training data
and evaluates on the test set.

Usage:
    python3 baseline_random_forest.py --train_file train_pure.csv --test_file test_split.csv \
        --seed 42 --save_metrics metrics_random_forest_baseline.json
"""
...
def main():
    ...
    parser.add_argument("--save_metrics", type=str, default="metrics_random_forest_baseline.json")
    ...
    if args.use_gridsearch:
        ...
        rf = RandomForestClassifier(random_state=args.seed, n_jobs=4)
        grid_search = GridSearchCV(rf, params_grid, scoring='f1_macro', cv=5, verbose=1, n_jobs=1)
        grid_search.fit(X_train, y_train)
        model = grid_search.best_estimator_
        print(f"Best params: {grid_search.best_params_}")
    else:
        print(f"Training Random Forest (n_estimators={args.n_estimators})...")
        model = RandomForestClassifier(n_estimators=args.n_estimators, random_state=args.seed, n_jobs=4)
