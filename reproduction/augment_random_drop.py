...
def augment_row(row, feature):
    ...
    if isinstance(val, str) and "," in val:
        ...
        # Logic: Randomly drop one item
        drop_idx = random.randint(0, len(items) - 1)
        new_items = items[:drop_idx] + items[drop_idx+1:]
...
def main():
    parser = argparse.ArgumentParser(description="Augment data using Random Drop method")
    ...
    print(f"Total augmented rows: {total_generated}")
    augment_utils.save_augmented_data(train_df, augmented_rows, "random_drop", args.strategy)
